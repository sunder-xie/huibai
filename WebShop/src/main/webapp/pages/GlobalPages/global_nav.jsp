...
<div class="global_nav_bar">
	<div class="goods_nav">
		...
		<div  id="global_wrap_nav" class="wrap_nav" style="display:none">
...
function createTree(jsons,pid){
    if(jsons != null){
        ...
        var icon = basepath+'images/logo/whale-256x256.png';
        var childlist ='';
        len = jsons.length;
        for(var i=0;i<len;i++){
...
function createTree2(index){	
	...
	if(havadata=='no')
	{
		...
		var requestUrl = basepath+'NavClassify/GetLastNavClassifyInfo.action';
		//导航兰的添加
...
//		alert(JSON.stringify(map));
		...
		var requestUrl = basepath+'Brand/GetBrandInfo.action';
		$.post(requestUrl,param,function(data){
//		alert(JSON.stringify(data));
			if(data.rspCode=="000000"){
				var ul='';
				...
				var len,brandNm,url;
				len = jsons.length;
				url = basepath+'pages/Search/SearchGoods.jsp?goods_id=';
	        	for(var i=0;i<len;i++){
	...
	//				imgurl='images/product/f1_01_02.png';
	    		...
	    		var lazyimg=basepath+"images/lazyimages/whale-120x50.png";
	    		$('img.ad-top-lazy').lazyload({placeholder : lazyimg,effect: "fadeIn"});
			}
		},"json");	
	}	
}
...
function createRightDown(index){
	...
	if(havadata=='no')
	{
		...
		for(i=0;i<2;i++)
		{
			...
			{
				ul +='<a href="'+'#'+'">';
				imgurl=basepath+'images/product/f1_01_02.png';
		...
		var lazyimg=basepath+"images/lazyimages/whale-245x120.png";
...
$(function(){
	...
	var requestUrl =basepath+ 'NavClassify/GetTopNavClassifyInfo.action';
	//导航兰的添加
	$.post(requestUrl,param,function(data){
	/* 	alert(JSON.stringify(data)); */
		if(data.rspCode=="000000"){
			...
			NavMenu+=item;
			NavMenu+='</div>';
			$("#global_wrap_nav").append(NavMenu);
			
			$('.global_nav_bar > .goods_nav').hover(function() {
				$(this).addClass('hover');
