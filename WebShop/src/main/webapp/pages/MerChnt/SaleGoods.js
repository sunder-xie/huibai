...
//	        }
	    ...
	    $(".reason").on("click",function(e){
	        ...
	        var reasonStr = '';
	        if( $(this).attr("reasonTit") == "3" ){
	            reasonStr = "审核不通过原因";
	        }else if( $(this).attr("reasonTit") == "5" ){
	            reasonStr = "违规下架原因";
	        }else if($(this).attr("reasonTit") == "6" ){
	            reasonStr = "强制下架原因";
	        }
...
function submitQuery(t){
	...
	if(t == 1){
		$('#queryForm').attr('action','/order/seller/orderList.html');
		$('#queryForm').submit();
		$('#submitQuery').attr('id','unbindClick');
		clearTimeout(clearTime);
		var clearTime=setTimeout('unbindClick()',5000);
...
function unbindClick(){
    $('#unbindClick').attr('id','submitQuery');
}
