...
var CheckExistCustomer = function(){
	var param = {
			userId:user_id,
			bindType:"01"
		};
...
//		alert(JSON.stringify(data));
		if(data.rspCode=="000000"){
			...
			window.location = basepath+'pages/User/customerInfo.jsp';
		}
    },"json");
}
...
//				$("#shopId").val(shopIdVal);
				$("#submitPersonShopBtn").parents(".item").slideToggle();
				$("#submitPersonShopBtn").parents(".item").next().slideToggle();
				var step = $("#submitPersonShopBtn").data("nextitem");
				$(".badge").removeClass("badge-info");
				$(".badge").addClass("badge-default");
				$("#"+step).children("span").removeClass("badge-default");
				$("#"+step).children("span").addClass("badge-info");
