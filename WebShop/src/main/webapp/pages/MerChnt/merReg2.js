...
//		},
	...
	$('.eprise_head_bd li').click(function(){
        var index = $(this).index();
        $('.eprise_body_bd .tab_aera').css('display','none');
        $('.eprise_body_bd .tab_aera').eq(index).show();
...
//	alert(JSON.stringify(param));
	if($('#regflow').attr("enter-type")=='1')
	{
		$('.eprise_body_bd .tab_aera').css('display','none');
        $('.eprise_body_bd .tab_aera').eq(0).show();
        
       // $('.eprise_head_bd ul li').addClass('active').siblings().removeClass('active');
...
$(function(){
	...
	$("#submitShopBtn").click(function(){
		...
		if($('#regflow').attr("reg-flow")=='1' && !isNull(shopIdVal))
		{
			...
			url = basepath+'Shop/UpdateShopInfo.action?userId='+user_id;
		}
		else
		{
			var param = {
			...
			url = basepath+'Shop/ShopRegister.action?userId='+user_id;
		}
		/* alert(JSON.stringify(param));
		alert(basepath); */
		$.post(url,param,function(data){
	...
	$("#submitPersonShopBtn").click(function(){
		var shopNmVal = $("#shopNm").val();
		var shopSalesVal = $("#shopSales").val();
		var shopAddrVal = $("#shopAddr").val();
		var shopTelVal = $("#shopTel").val();
	...
	$("#submitEpriseBtn").click(function(){
		...
		if($(".person_info").is(":hidden"))	/*个人信息隐藏*/
		{
...
//					setTimeout(goToMerCenter, 5000);
		...
		if($('#regflow').attr("reg-flow")=='1' && !isNull(custId))
		{
			var param = {
					shopId:shopIdVal,
					shopNm:shopNmVal,
			...
			url = basepath+'Shop/UpdateShopInfo.action';
		}
		else
		{
			var param = {
			...
			url = basepath+'Shop/ShopRegister.action';
		}
		/* alert(JSON.stringify(param));
		alert(basepath); */
		$.post(url,param,function(data){
