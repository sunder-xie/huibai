...
$(function(){
    ...
    getMerOrder(startpagenum,offset);
});
...
var getMerOrder=function(pagenumber,offsize){
	var map={
		"shopId":shop_id
	};
	var param = {
	...
	var url =basepath+ "Order/GetOrderInfoBySaler.action?userId="+user_id;
	var datatable='';
...
//		alert(JSON.stringify(data));
	    ...
	    for (var i=0;i<num;i++)
		{
			...
			var userNm = data.result[i].userNm;
			...
			{
				datatable+='  <a  class="btnMinR cm-btnPay mb10" href="javascript:openConfirmReceive(\'1100000014588645\');">待付款</a>';
			}else if(orderStatus=='02')
			{
				datatable+='  <a  class="btnMinR cm-btnPay mb10">立即发货</a>';
			}
			else if(orderStatus=='03')
			{
				datatable+='  <a  class="btnMinR cm-btnPay mb10" href="javascript:openConfirmReceive(\'1100000014588645\');">待收货</a>';
			}
			else if(orderStatus=='04')
			{
				datatable+='  <a  class="btnMinR cm-btnPay mb10">交易完成</a>';
			}
			else /*if(orderStatus=='05')*/
			{
				datatable+='  <a  class="btnMinR cm-btnPay mb10">交易关闭</a>';
			}
			datatable+='</div>';
			datatable+=' <div class="cl_both"></div>';
			datatable+=' <div class="order_oper bus_message">';
			datatable+=' <a class="bus_name" >';
			datatable+='   <em style="color:#888">买家：</em>'+userNm+'</a>';
...
var PageClick = function(pageclickednumber) {  
	getMerOrder(pageclickednumber,offset);  
}  
