...
var getOrderLists = function(){
    ...
    $.post('Order/GetOrderListBySaler.action',param,function(data){
...
var genOrderInfo = function(orders){
	var orderId,totalAmt,totalNum,shopId,shopNm,shopUrl,status,staDesc,userNm;
	...
	{
		staDesc='等待卖家发货';
		opertion +=' <a href="javascript:sendOrder(\''+orderId+'\')"><span class="gary order-opertion">发货</span></a>';
	}else if(status=='03')
	{
		staDesc='待收货';
	}else if(status=='04')
	...
	$('#container').append(empty.template(orderId,userNm,staDesc,totalNum,totalAmt,opertion));
...
var delOrder = function(orderId){
	...
	$.post(url,param,function(data){
		if(data.rspCode=="000000"){
			...
			window.location = "pages/merchnt/merOrder.jsp?shop_id=" +shop_id;
		}else{
